...
vi.mock("../../../src/services/payment/MobbexPaymentService", () => ({
  MobbexPaymentService: class {
...
vi.mock("../../../src/middlewares/validateWebhook", () => ({
  validateMobbexWebhook: (_req: any, _res: any, next: any) => next(),
}));

import { mobbexWebhookRouter } from "../../../src/routes/webhooks/mobbex";
...
describe("POST /api/webhooks/mobbex", () => {
  it("responde 200 inmediatamente, antes de procesar", async () => {
    ...
    const res = await request(makeApp())
      ...
      .send({ type: "payment", data: { payment: { id: "p1" } } });
  ...
  it("rejected → PAGO_FALLIDO con history, sin notificar", async () => {
    ...
    await request(makeApp())
      .post("/api/webhooks/mobbex")
      .send({ type: "payment", data: { payment: { id: "p2" } } });
    await tick();
  ...
  it("orphan payment: order no encontrada → loguea y termina sin throw", async () => {
    ...
    const res = await request(makeApp())
      .post("/api/webhooks/mobbex")
      .send({ type: "payment", data: { payment: { id: "p" } } });
    expect(res.status).toBe(200);
  });
